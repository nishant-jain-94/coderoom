...
const load = async () => {
  ...
  const membersOfGroup = await
    Promise.all(INCLUDED_GROUPS.map(getMembersOfGroupAsync.bind(null, GITLAB_URL, API_TOKEN)));
  const members = [].concat(...membersOfGroup);
  // console.log(members);
  const mentorsGroup = await
    Promise.all(EXCLUDED_GROUPS.map(getMembersOfGroupAsync.bind(null, GITLAB_URL, API_TOKEN)));
