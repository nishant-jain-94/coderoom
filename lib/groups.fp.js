 ...
 * @param {*} gitlab
 */
const listAllGroups = gitlab => promisify(gitlab.groups.all);
...
const getAllGroupsAsChoices = R.compose(
  R.composeP(
    R.compose(Promise.resolve.bind(Promise), R.map(transformGroupsAsChoice)),
    listAllGroups,
  ),
  getGitlabInstance,
);
 ...
 */
module.exports = R.memoize(getAllGroupsAsChoices);
